import Dropn from "./Dropn.cdc"
import Nxt from "./Nxt.cdc"
import FungibleToken from "./FungibleToken.cdc"
import NonFungibleToken from "./NonFungibleToken.cdc"

/*
    This is a simple Nxt initial sale contract for the DApp to use
    in order to list and sell Nxt.
...
pub contract NxtMarket {
    ...
    // SaleOffer
    // A Nxt NFT being offered to sale for a set fee paid in Dropn.
    //
    pub resource SaleOffer: SaleOfferPublicView {
        ...
        access(self) let sellerItemProvider: Capability<&Nxt.Collection{NonFungibleToken.Provider}>

        // The Dropn vault that will receive that payment if teh sale completes successfully.
        access(self) let sellerPaymentReceiver: Capability<&Dropn.Vault{FungibleToken.Receiver}>

        // Called by a purchaser to accept the sale offer.
        // If they send the correct payment in Dropn, and if the item is still available,
        // the Nxt NFT will be placed in their Nxt.Collection .
        //
        pub fun accept(
            buyerCollection: &Nxt.Collection{NonFungibleToken.Receiver},
        ...
        // to transfer the Nxt NFT and the capability to receive Dropn in payment.
        //
        init(
            sellerItemProvider: Capability<&Nxt.Collection{NonFungibleToken.Provider}>,
            itemID: UInt64,
            typeID: UInt64,
            sellerPaymentReceiver: Capability<&Dropn.Vault{FungibleToken.Receiver}>,
            price: UFix64
        ) {
            pre {
    ...
    pub resource interface CollectionManager {
        pub fun insert(offer: @NxtMarket.SaleOffer)
    ...
    pub resource Collection : CollectionManager, CollectionPurchaser, CollectionPublic {
        ...
        // purchase
        // If the caller passes a valid itemID and the item is still for sale, and passes a Dropn vault
        // typed as a FungibleToken.Vault (Dropn.deposit() handles the type safety of this)
    ...
    init () {
        ...
        self.CollectionStoragePath = /storage/nxtMarketCollection002
        self.CollectionPublicPath = /public/nxtMarketCollection002
    }
}
